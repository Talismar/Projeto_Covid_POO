...
class Prediction:
	"""
	Receives a column specified by the user (from the entire dataset) and returns a prediction of the next month
	if there will be an increase or decrease in the face of the trained data.
	"""
	...
	def polynomialRegression(self, dataset, numCol, show=False):
		"""
			...
			If show is true it saves the image.
	...
	def prediction(self, numUp):
		...
		#If the user wants to see the value predicted by the algorithm, just uncomment the codes
